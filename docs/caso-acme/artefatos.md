...
| --- | --- | --- | --- |
| 1 | Mapa de atores e restrições, com identificação de quem decide cada classe de questão | Mapa de atores e restrições fechadas da [página inicial do caso](index.md) | Cada restrição aparece ligada ao ator que a impôs, e ao menos um conflito de interesse é nomeado com a autoridade que o resolve |
| 2 | Classificação dos requisitos declarados em requisito funcional, requisito de atributo de qualidade, requisito não funcional de outra natureza e restrição, com reescrita de dois itens em forma mensurável | Lista de requisitos R1 a R15 da [página inicial do caso](index.md) | Os 15 itens classificados com justificativa de uma linha, e os itens reescritos passam a declarar estímulo, resposta e medida |
...
Os quatro artefatos da Aula 1 formam o enquadramento do problema. Eles respondem quem quer o quê, o que é exigência e o que é restrição, como medir a exigência e qual organização estrutural atende melhor às exigências medidas.
...
A dependência entre artefatos define a ordem em que eles podem ser produzidos. A cadeia da Aula 1 é linear. A classificação de requisitos depende do mapa de atores, porque a origem de um requisito determina se ele é negociável. Os cenários dependem da classificação, porque só requisito de atributo de qualidade vira cenário. A comparação de estilos depende dos cenários, porque estilo se avalia contra exigência medida, não contra exigência declarada em termos gerais.
